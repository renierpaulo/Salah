/*
 * HYPERION SEARCH ULTRA - Maximum Performance Full Pipeline
 * Based on the proven 17 GKeys/s profiler kernel with affine batch inversion
 * 
 * PIPELINE:
 * [GPU] privkey++ → Affine EC add (batch inv) → HASH160 → Bloom → buffer
...
// Main search kernel - based on proven 17 GKeys/s profiler kernel
__global__ void __launch_bounds__(256, 4)
kernel_search_ultra(uint64_t* __restrict__ d_px, uint64_t* __restrict__ d_py,
    ...
    // Phase 3: Process all points with EC add + hash + bloom
    #pragma unroll 16
...
int main(int argc, char** argv) {
    printf("\n");
    printf("╔══════════════════════════════════════════════════════════════╗\n");
    printf("║     HYPERION SEARCH ULTRA - 17+ GKeys/s Full Pipeline     ║\n");
    ...
    printf("  Targets:    %s\n", targets_file);
    printKey256("  Start:      0x", rs);
    printKey256("  End:        0x", re);
    uint64_t kpl = (uint64_t)NUM_THREADS * BATCH_SIZE;
    ...
    uint64_t *d_px, *d_py;
    cudaMalloc(&d_px, NUM_THREADS * 4 * sizeof(uint64_t));
    cudaMalloc(&d_py, NUM_THREADS * 4 * sizeof(uint64_t));
    ...
    while (true) {
        kernel_search_ultra<<<NUM_BLOCKS, THREADS_PER_BLOCK>>>(d_px, d_py, d_bloom, batch_lo, d_cnt);
            ...
            if (mc > 0) {
                ...
                for (int i = 0; i < mc && i < MAX_MATCHES; i++) {
                    ...
                    if (verifyMatch(hm[i], h_target_set)) {
                        verified++;
                        printf("\n🎯 MATCH #%d!\n", verified);
                        printKey256Hex("  Key:  ", hm[i].key);
                        printHash160("  Hash: ", hm[i].hash160);
                    }
                }
                delete[] hm;
                zero = 0;
        ...
        if (iteration % 100 == 0) {
            ...
            printf("\r[%.2f GKeys/s] %llu keys | bloom:%d | match:%d   ", speed, total, total_bloom, verified);
            fflush(stdout);
        }
    ...
    printf("  Bloom: %d | Confirmed: %d\n", total_bloom, verified);
    printf("═══════════════════════════════════════════════════════════════\n\n");
