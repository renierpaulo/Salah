/*
 * HYPERION SPEED - Simple Fast Search (no complex shared memory)
 * Based on profiler architecture: 1 base point per thread, affine coordinates
 */
...
// Simple kernel - each thread processes BATCH_SIZE points with local batch inversion
__global__ void __launch_bounds__(256, 3)
kernel_speed(uint64_t* __restrict__ d_px, uint64_t* __restrict__ d_py,
...
int main(int argc, char** argv) {
    printf("\n╔══════════════════════════════════════════════════════════════╗\n");
    printf("║          HYPERION SPEED - Fast Bitcoin Key Search          ║\n");
    printf("╚══════════════════════════════════════════════════════════════╝\n\n");
    ...
    int iter = 0;
    while (true) {
        kernel_speed<<<NUM_BLOCKS, THREADS_PER_BLOCK>>>(d_px, d_py, d_bloom, cur[0], d_cnt);
