/*
 * HYPERION PTX - Ultra-optimized search using PTX inline assembly
 * Target: 18+ GKeys/s through optimized Point Addition and Batch Inversion
 */
...
// Scalar multiplication for init (uses standard functions)
...
// Ultra-optimized kernel using PTX operations
__global__ void __launch_bounds__(256, 4)
kernel_ptx(uint64_t* __restrict__ d_px, uint64_t* __restrict__ d_py, 
    ...
    // Phase 1: Build products for batch inversion using PTX
    uint64_t products[BATCH_SIZE][4];
    uint64_t dx[4];
    
    fieldSub_PTX(&d_G_multiples_x[0], base_x, dx);
    #pragma unroll
    for(int j=0; j<4; j++) products[0][j] = dx[j];

    #pragma unroll 32
    for(int i=1; i<BATCH_SIZE; i++) {
        fieldSub_PTX(&d_G_multiples_x[i*4], base_x, dx);
        fieldMul_PTX(products[i-1], dx, products[i]);
    }

    // Single Fermat inversion using PTX
    uint64_t inv[4];
    fieldInv_PTX(products[BATCH_SIZE-1], inv);

    uint64_t inv_dx_last[4];

    // Phase 2: Process all points using PTX operations
    #pragma unroll 32
    for(int i=BATCH_SIZE-1; i>=0; i--) {
        ...
        if(i == 0) {
            ...
            fieldMul_PTX(inv, products[i-1], inv_dx);
        }
        ...
        uint64_t dy[4];
        fieldSub_PTX(gy, base_y, dy);
        
        uint64_t lambda[4];
        fieldMul_PTX(dy, inv_dx, lambda);
        
        uint64_t lambda_sq[4], temp[4], x3[4];
        fieldSqr_PTX(lambda, lambda_sq);
        fieldSub_PTX(lambda_sq, base_x, temp);
        fieldSub_PTX(temp, gx, x3);
        
        uint64_t y3[4];
        fieldSub_PTX(base_x, x3, temp);
        fieldMul_PTX(lambda, temp, y3);
        fieldSub_PTX(y3, base_y, y3);
    ...
    fieldSub_PTX(gx_last, base_x, dx_last);
    fieldSub_PTX(gy_last, base_y, dy_last);
    fieldMul_PTX(dy_last, inv_dx_last, lambda);
    fieldSqr_PTX(lambda, lambda_sq);
    fieldSub_PTX(lambda_sq, base_x, temp);
    fieldSub_PTX(temp, gx_last, base_x);
    
    uint64_t old_base_x[4];
    #pragma unroll
    for(int i=0; i<4; i++) old_base_x[i] = d_px[tid*4 + i];
    
    fieldSub_PTX(old_base_x, base_x, temp);
    fieldMul_PTX(lambda, temp, base_y);
    fieldSub_PTX(base_y, &d_py[tid*4], base_y);
...
int main(int argc, char** argv) {
    printf("\n╔══════════════════════════════════════════════════════════════╗\n");
    printf("║       HYPERION PTX - Ultra-Optimized (Target: 18+ GKeys/s)  ║\n");
    printf("╚══════════════════════════════════════════════════════════════╝\n\n");
    ...
    printf("SEARCHING (PTX optimized)...\n");
    fflush(stdout);
    ...
    int iter = 0;
    while (true) {
        kernel_ptx<<<NUM_BLOCKS, THREADS_PER_BLOCK>>>(d_px, d_py, batch_lo, d_cnt);
