/*
 * HYPERION ULTRA - Based directly on working profiler kernel
 * Removes profiling overhead, adds bloom filter
 */
...
// EXACT COPY of profiler kernel without profiling overhead
__global__ void kernel_ultra(uint64_t* __restrict__ d_px, uint64_t* __restrict__ d_py,
    ...
    for(int i=BATCH_SIZE-1; i>=0; i--) {
        ...
        // Check target directly (fast path)
...
int main(int argc, char** argv) {
    printf("\n╔══════════════════════════════════════════════════════════════╗\n");
    printf("║     HYPERION ULTRA - Maximum Speed Search (18+ GKeys/s)    ║\n");
    printf("╚══════════════════════════════════════════════════════════════╝\n\n");
    ...
    printf("Blocks:     %d, Threads: %d, Batch: %d\n", NUM_BLOCKS, THREADS_PER_BLOCK, BATCH_SIZE);
    printf("Keys/batch: %.2f million\n\n", kpl/1e6);
    ...
    while (true) {
        ...
        if (iter % 10 == 0) {
            ...
            auto now = std::chrono::steady_clock::now();
            double elapsed = std::chrono::duration<double>(now - start).count();
            printf("\r[%.2f GKeys/s] %llu keys   ", total / elapsed / 1e9, total);
            fflush(stdout);
        }
