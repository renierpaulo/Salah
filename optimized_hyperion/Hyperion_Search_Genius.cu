/*
 * HYPERION SEARCH GENIUS - Based on Profiler Genius (16+ GKeys/s)
 * Modified for continuous search instead of fixed 50 iterations
 */
...
// EXACT kernel from profiler - proven 16+ GKeys/s
__global__ void __launch_bounds__(256, 4)
kernel_search_genius(uint64_t* __restrict__ d_px, uint64_t* __restrict__ d_py, 
    ...
    for(int i=BATCH_SIZE-1; i>=0; i--) {
        ...
        // Fast prefix check then full compare
...
int main(int argc, char** argv) {
    printf("\n╔══════════════════════════════════════════════════════════════╗\n");
    printf("║    HYPERION SEARCH GENIUS - 16+ GKeys/s Continuous Search   ║\n");
    printf("╚══════════════════════════════════════════════════════════════╝\n\n");
    ...
    printf("Keys/iter:  %.2f million (536.87M)\n\n", kpl/1e6);
    ...
    printf("SEARCHING at 16+ GKeys/s...\n");
    fflush(stdout);
    ...
    int iter = 0;
    while (true) {
        kernel_search_genius<<<NUM_BLOCKS, THREADS_PER_BLOCK>>>(d_px, d_py, batch_lo, d_cnt);
